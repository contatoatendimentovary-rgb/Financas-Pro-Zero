...
window.carregarDados = async () => {
    ...
    trans.forEach((t, idx) => {
        ...
        html += `<div class="entry-card"><div><b>${t.descricao}</b><br><small>${t.tipo}</small></div>
        <div style="text-align:right"><span style="color:${t.tipo==='entrada'?'var(--green)':'var(--text-main)'}">${t.tipo==='entrada'?'+':'-'} R$ ${v.toFixed(2)}</span><br>
        <button class="btn-del" onclick="deletar(${idx})">Remover</button></div></div>`;
    });
    ...
    document.getElementById('lista').innerHTML = html || '<p style="text-align:center;opacity:0.5">Vazio</p>';
    updateBars(r, f, l, i);
    renderPizza(f, l, i);
};
...
document.getElementById('btnSalvar').onclick = async () => {
    ...
    const d = document.getElementById('desc').value, v = document.getElementById('valor').value, t = document.getElementById('tipo').value, m = document.getElementById('filtroMes').value;
    if (!user || !d || !v) return;
    const ref = window.doc(window.db, "usuarios", user.uid, "meses", m);
    const snap = await window.getDoc(ref);
    let trans = snap.exists() ? snap.data().transacoes : [];
    trans.push({ descricao: d, valor: parseFloat(v), tipo: t });
